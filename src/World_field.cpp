...
Cell& World_field::operator()(size_t i, size_t j) {
	if (i < size_row_ && j < size_col_ && this != nullptr) {
		return array_[i][j];
	}
...
void World_field::count_living_neighbors_for_each_cell() {
	for (size_t i = 0; i != size_row_; i++) {
		for (size_t j = 0; j != size_col_; j++) {
			array_[i][j].set_living_neighbors() = cout_living_neighbors(*this, i, j);
		}
	}
}
		...
		for (size_t j = 0; j != size_col_; j++) {
			if (array_[i][j].is_life()) {
				if (array_[i][j].living_neighbors() > 3 || array_[i][j].living_neighbors() < 2) {
					next_world(i,j).set_life() = false;
				}
			}
			else {
				if (array_[i][j].living_neighbors() == 3) {
					next_world(i, j).set_life() = true;
				}
			}
		}
	}
...
unsigned cout_living_neighbors(World_field const& World, int const& n_row, int const& n_col) {
	unsigned cnt = 0;
